...
{
	$GLOBALS['Recursos']->paginacao($GLOBALS['Recursos']->query($GLOBALS['selects']->all_produtos()), $GLOBALS['selects']->all_produtos(), $pagina, function ($dados) {
		echo "<tr>
					<td>" . $dados[0] . "</td>
					<td>" . $dados[1] . "</td>
					...
					<td>" . $dados[3] . "</td>
					<td>" . $dados[4] . "</td>
					 <td><a onclick=janela_mensagem('Editar','produto','../../php/request.php?page=editar/produto&id=" . $dados[0] . "')><i class='material-icons'>edit</i></a></td>
						<td><a onclick=janela_mensagem('Deletar','Produto','../../php/request.php?page=deletar/produto&id=" . $dados[0] . "')><i class='material-icons  status_indisponível'>delete</i></a></td>
				</tr>";
	}, function ($paginas) {
		if ($paginas > 1) {
			for ($i = 1; $i < $paginas + 1; $i++) {
				echo "
					<div class='paginacao'><a  href='../../pages/relatorios/relatorio_todos_produtos?pagina=$i'>" . $i . "</a></div>
				";
			}
		}
	});
