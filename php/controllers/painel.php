...
function estatisticas($estatistica)
{
	if ($estatistica == 'troca') {
		...
		return $GLOBALS['Recursos']->contador_registros($GLOBALS['selects']->all_saida_manutencoes());
...
function substituicao_veiculos()
{
	$GLOBALS['Recursos']->fetch_array(
		$GLOBALS['Recursos']->query($GLOBALS['selects']->all_veiculos_substituicao()),
		function ($dados) {
			echo "<tr class='danger'>
						<td>" . $dados[6] . "</td>
						<td>" . $dados[9] . "</td>
						<td>" . $dados[5] . "</td>
						<td>" . $GLOBALS['Recursos']->data_diferenca($dados[5], date('Y-m-d H:i:s')) . "</td>
						<td>" . $dados[4] . "</td>
				</tr>";
		}
	);
